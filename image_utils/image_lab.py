...
def test_img_lab_diff():
    from torchvision import transforms
    trm=transforms.Compose([transforms.ToTensor(),])
    in_img=trm(Image.open('Dengjiacheng.png').convert('RGB'))
    in_img=in_img.unsqueeze(0)
    noise=torch.rand(*in_img.shape)*0.1
    device=torch.device('cpu')
    input_LAB=rgb2lab_diff(in_img,device)
    output_LAB=rgb2lab_diff(torch.clip(in_img+noise,0,1),device)
    diff=ciede2000_diff(input_LAB,output_LAB,device)
    print(diff.mean())
    return
